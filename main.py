...
def start_books(message):
    ...
    mass_books = [book_0, book_1, book_2, book_3,
                  book_4, book_5, book_6, book_7, book_8, book_9]
    description_book = [book_d_0, book_d_1, book_d_2, book_d_3,
                        book_d_4, book_d_5, book_d_6, book_d_7, book_d_8, book_d_9]
...
def want_smoke_message(message):
    ...
    WS.append(
        'Попросите своих близких людей в помощи с бросанием никотиновой зависимости!')
    WS.append(
        'Чтобы не срываться, постарайтесь полностью избавиться от табачной продукции в своем окружении')
    ...
    WS.append(
        'Сигареты очень плохо пахнут, это еще одна из причин не курить сигареты.')
    WS.append(
        'Попробуйте отвлечься от желания закурить чем-то действительно интересным!')
    WS.append('Если Вы хотите покурить, то сходите и попейте чай, ну правда')
    WS.append('В 90% случаях рак легких вызван курением...')
    WS.append(
        'Если Вы не задумывались над страшными картинками на пачках сигарет, то пора задуматься...')
    WS.append(
        'Если Вы также пьете алкоголь, то курение вызывает в несколько раз более пагубные последствия(')
    WS.append(
        'Все получится! 60% людей, бросивших курить утверждают, что это было легче, чем они думали!')
    WS.append(
        'Для ароматизации в сигареты добавляется мочевина [сами погуглите что это такое)]')
    ...
    WS.append(
        'В среднем пачка сигарет стоит 120 рублей, на эти деньги можно купить шоколад ritter sport...')
...
def func(message):
    ...
    if (message.text == "Здоровье💊"):
        ...
        bot.send_message(message.chat.id, "Не переживайте это нормально, что иногда не получается удержаться в челлендже. Самое главное, не ругайте себя, вы уже молодец, что начали продвижение к жизни без курения! Давайте ещё постараемся❤️",
                         reply_markup=telebot.types.ReplyKeyboardRemove())
        bot.send_sticker(
...
def schedule_func():  # как часто пользователи будут получать сообщение вы курили?
    schedule.every(1).days.do(notification)
    while run_schedule:
        time.sleep(1)
